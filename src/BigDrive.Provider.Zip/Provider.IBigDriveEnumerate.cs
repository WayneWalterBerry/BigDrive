...
namespace BigDrive.Provider.Zip
{
    ...
    /// <summary>
    /// Implementation of <see cref="BigDrive.Interfaces.IBigDriveEnumerate"/> for the Zip provider.
    /// Enumerates folders and files within the ZIP archive.
    /// </summary>
    public partial class Provider
    {
        /// <summary>
        /// Enumerates the folder names at the specified path within the ZIP archive.
        /// </summary>
        ...
        public string[] EnumerateFolders(Guid driveGuid, string path)
        {
            try
            {
                ...
                return zipClient.GetFolders(NormalizePath(path));
            }
            catch (Exception ex)
            {
        ...
        /// <summary>
        /// Enumerates the file names at the specified path within the ZIP archive.
        /// </summary>
        /// <param name="driveGuid">The drive GUID.</param>
        ...
        public string[] EnumerateFiles(Guid driveGuid, string path)
        {
            try
            {
                ...
                ZipClientWrapper zipClient = GetZipClient(driveGuid);
                return zipClient.GetFiles(NormalizePath(path));
            }
            catch (Exception ex)
            {
