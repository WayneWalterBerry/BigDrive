...
namespace BigDrive.Provider.Archive
{
    ...
    /// <summary>
    /// Implementation of <see cref="BigDrive.Interfaces.IBigDriveFileData"/> for the Archive provider.
    /// Provides file data from archive files (ZIP, TAR, 7z, RAR, etc.) as a COM IStream.
    /// </summary>
    public partial class Provider
    {
        /// <summary>
        /// Gets the file data as a stream for the file at the specified path within the archive.
        /// </summary>
        ...
        public int GetFileData(Guid driveGuid, string path, [MarshalAs(UnmanagedType.Interface)] out IStream stream)
        {
            ...
            try
            {
                ...
                byte[] fileData = archiveClient.GetFileData(NormalizePath(path));
